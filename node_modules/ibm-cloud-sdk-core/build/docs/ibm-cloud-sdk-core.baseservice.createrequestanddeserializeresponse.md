...
see `parameters` in `createRequest`
...
the deserializer function that is applied on the response object
...
_(Optional)_ is `true` when the response object should be handled as a map
